...
class PCATransformer(sklearn.base.TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        if self._n_components <= 10:
            ...
            mu = np.mean(X, axis=0)
            S = (1/(X.shape[0])) * (X - mu).T @ (X - mu)
            V = np.zeros((S.shape[1], self._n_components))
            ...
            for i in range(self._n_components):
                ...
                for _ in range(iterations):
                    v = S @ v
                    lambd = np.linalg.norm(v)
                    v = v/lambd
                V[:,i] = v
                S = S - lambd * np.outer(v,v)

        else:
            # TODO: Use the SVD decomposition computed with `np.linalg.svd`
            # to find the principal components.
            mu = np.mean(X, axis=0)
            u, s, vh = np.linalg.svd(X - mu, full_matrices=True)
            V = vh[:self._n_components].T
