...
def cross_entropy(y_pred, target):
    ...
    H_pq = -1*target*np.log(y_pred) - (1-target) *np.log(1-y_pred)  
    H_pq = np.sum(H_pq)/len(y_pred)   # normalization
...
def main(args: argparse.Namespace): # -> tuple[np.ndarray, list[tuple[float, float]]]:
    ...
    # DONE TODO: Append a constant feature with value 1 to the end of every input data
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(0, len(permutation), args.batch_size): 
            ...
            for j in range(args.batch_size):
                # grad = (y(x) - t) * x....  y = sigmoid(x) = 1/(1-exp(y))   
                grads[:,j] =  (sigmoid(xb_T[j,:] @ weights) - tb[j]) * xb_T[j,:]         
        ...
        y_test, y_train = sigmoid(test_data @ weights), sigmoid(train_data @ weights) 
        
        train_accuracy = accuracy(y_train, train_target)
        test_accuracy  = accuracy(y_test,  test_target)
        train_loss = cross_entropy(y_train, train_target)
        test_loss = cross_entropy(y_test,  test_target)
